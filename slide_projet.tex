...
% --- Données ---
\title[Méthode GLV]{Analyse et Implémentation de la Méthode GLV sur Courbes Elliptiques}
...
\begin{frame}{La Solution GLV (Gallant-Lambert-Vanstone)}
    ...
    \textbf{Principe :} Décomposer le grand scalaire $k$ en deux scalaires $k_1, k_2$ de taille moitié ($\approx 128$ bits).
...
\begin{frame}{Courbes Elliptiques et Coordonnées}
    ...
    \begin{columns}
        ...
        \begin{exampleblock}{Coordonnées Jacobiennes $(X:Y:Z)$}
            \begin{itemize}
                \item Changement de variables $x = X/Z^2, y = Y/Z^3$.
                \item \textbf{Aucune inversion} dans la boucle.
                \item Réduit le coût du doublement.
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Les Endomorphismes GLV ($\phi$)}
    \begin{block}{Définition}
        Un endomorphisme est une application rationnelle $\phi : E \to E$. Restreint à un sous-groupe cyclique d'ordre $n$, $\phi$ agit comme la multiplication par $\lambda$ :
...
\begin{frame}{Classification : Les Cas Complexes}
    ...
    \begin{alertblock}{Type 3 : Courbes avec $D=7$ (Implémenté)}
        Défini sur $\mathbb{Q}(\sqrt{-7})$. $\zeta=(1+\sqrt{-7})/2,\, a=(\zeta-3)/4$
        $$ \phi(x,y) = \left(\frac{x^2 - \xi}{\xi^2(x-a)}, \zeta^{-3}y\frac{x^2-2ax+\zeta}{(x-a)^2}\right) $$
    \end{alertblock}
...
\begin{frame}{Focus : La Complexité du Type 3}
    ...
    L'ordre $N$ n'est pas toujours premier :
    $$ N = h \cdot n \quad \text{avec } h \text{ un cofacteur petit (typiquement, } h=2 \text{ ou } h=4)$$
    
    \begin{alertblock}{Conséquence critique}
        GLV ne fonctionne bien que sur le sous-groupe d'ordre premier $n$.
        \begin{itemize}
            \item Il faut travailler modulo $n$.
...
\begin{frame}{Le Problème de Décomposition}
    ...
    \begin{enumerate}
        \item Trouver une base de vecteurs court $v_1, v_2$ du réseau (Pré-calcul).
...
\begin{frame}{Étape 1 : Génération de la Base (Euclide Étendu)}
    ...
    \begin{itemize}
        \item On génère une suite $s_i\cdot n+t_i\cdot\lambda=r_i$.
        \item On s'arrête quand $r_m < \sqrt{n}$.
    \end{itemize}
...
\begin{frame}{Étape 2 : Décomposition de Babai (Rounding)}
    Pour tout scalaire $k$, on résout le système dans $\mathbb{Q}$ :
    $$ (k,0) = c_1 v_1 + c_2 v_2 $$
...
\begin{frame}{L'Astuce de Shamir (Multi-exponentiation)}
    Une fois $k$ décomposé, on a $kP=(k_1+k_2\lambda)P=k_1P + k_2\phi(P)$. Comment le calculer vite ?
...
\begin{frame}{Architecture Modulaire (C / GMP)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Bas niveau :} \texttt{EC\_add\_proj.c, EC\_struct.c} (Optimisé GMP).
    \end{itemize}
...
\end{lstlisting}
    ...
    \textbf{Compromis :} $w=2$ est optimal. Au-delà ($w\geq4$), le temps de pré-calcul de la table devient trop lourd.
\end{frame}
...
\begin{frame}{Méthodologie de Validation}
    ...
    \begin{enumerate}
        \item \textbf{Génération des constantes :} Script SageMath pour trouver des nombres premiers et des courbes elliptiques convenables, puis calcul de $\lambda$ et $\beta$.
        ...
        \begin{itemize}
            ...
            \item \textbf{Succès si :} $Q_1 == Q_2$ (bit à bit).
        \end{itemize}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Analyse de la Performance}
    ...
    \textbf{Pourquoi pas 2.0× ?}
\begin{itemize}
    \item 2.0× = borne théorique asymptotique (endomorphisme gratuit, pas d’overhead)
    \item Coûts fixes : conversions + décomposition GLV (Babai)
    \item Mémoire/cache : tables et accès > coût des additions
    \item Précalculs pouvant être diminué 
\end{itemize}
...
\begin{frame}{Conclusion}
    \begin{enumerate}
        ...
        \item \textbf{Robustesse :} Gestion mémoire sans fuite et rigueur mathématique.
    \end{enumerate}

    \vspace{0.5cm}
    \begin{block}{Perspectives}
        Amélioration de l'implémentation: utilisation de variables statiques pour ne pas faire d'initialisation dans des boucles.
        
        Resistance aux attaques (Timing Attacks, Differential Power Analysis...)
    \end{block}
\end{frame}

%\begin{frame}{Perspective : Joint Sparse Form (JSF)}
%    Pour aller plus loin que le fenêtrage ($w=2$), la littérature suggère la JSF.
    
%    \textbf{Principe :}
%    Représentation binaire signée conjointe de $k_1$ et $k_2$ avec coefficients $\{-1, 0, 1\}$.
    
%    \begin{block}{Gain théorique}
%        Réduit la densité des termes non-nuls de $0.75$ à $0.5$.
%        $\Rightarrow$ Encore moins d'additions de points, mais implémentation plus complexe.
%    \end{block}
%\end{frame}
