...
\begin{remarque}
Ces formules nécessitent le calcul de l'inverse modulaire du dénominateur pour obtenir $\lambda$. Cette opération d'inversion est très coûteuse en temps de calcul (environ 20 à 80 multiplications), ce qui motive l'utilisation de systèmes de coordonnées alternatifs pour l'éviter (voir section suivante).
\end{remarque}
...
Nous notons :
\begin{itemize}
    \item \textbf{M} : Multiplication dans $\mathbb{F}_p$
    \item \textbf{S} : Mise au carré (Squaring) dans $\mathbb{F}_p$ ($1S \approx 0.8M$)
    \item \textbf{I} : Inversion modulaire. C'est l'opération critique, coûtant entre \textbf{30 et 80 M} selon l'architecture.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Élimination de l'Inversion :} Contrairement à l'affine, on ne fait aucune inversion $I$ pendant la boucle. On en fait une seule à la toute fin (Conversion Finale).
...
\begin{remarque}
Il est crucial de distinguer $\alpha$ et $\lambda$ :
\begin{itemize}
    \item $\alpha$ est une constante du corps de base $\mathbb{F}_p$ utilisée pour calculer les nouvelles coordonnées.
    \item $\lambda$ est l'entier correspondant dans le groupe scalaire.
\end{itemize}
L'intérêt de la méthode réside dans le fait que l'opération géométrique $(-x, \alpha y)$ ne coûte qu'une seule multiplication dans $\mathbb{F}_p$, remplaçant ainsi instantanément la multiplication par le grand scalaire $\lambda$.
\end{remarque}
\newpage
...
Si nous trouvons un tel $v$, alors le vecteur différence $u = (k, 0) - v = (k_1, k_2)$ sera court (sa norme euclidienne est petite), et par construction :
...
\fbox{
    ...
    \small \textit{Point d'entrée, mesures de temps et tests} \\
    \texttt{main.c, gen\_const\_*.sage.py}}
}

% Jonction 1
\par\nointerlineskip\vspace{-0.1cm} % Ajustement haut de la flèche
$\downarrow$
\par\nointerlineskip\vspace{-0.3cm} % Ajustement bas de la flèche
...
\fbox{
    ...
    \texttt{EC\_GLV.c, glv\_decompose.c, short\_vectors.c}}
}
...
\fbox{
    ...
    \texttt{EC\_add\_proj.c, EC\_struct.c, EC\_conversions.c}}
}
...
L'inversion modulaire est l'opération la plus coûteuse dans un corps fini $\mathbb{F}_p$ (coûtant environ 100 fois plus cher qu'une multiplication). Dans une multiplication scalaire classique, chaque addition de points nécessite une inversion pour calculer la pente de la droite ($\lambda$).
...
Pour calculer la combinaison linéaire $k_1P + k_2Q$, nous n'effectuons pas deux multiplications séparées. Nous utilisons une méthode d'entrelacement (Interleaving) combinée à du \textbf{fenêtrage (Windowing)} de taille $w$.
...
Pour les courbes $j=0$ (Type 2) et $j=1728$ (Type 1), l'opération se résume à une simple multiplication modulaire d'une coordonnée par une racine de l'unité $\beta$.
...
\begin{itemize}
    ...
    \item \textbf{Reproductibilité :} Les benchmarks de performance (dans \texttt{main.c}) utilisent un générateur aléatoire \texttt{gmp\_randstate\_t} initialisé avec l'entropie système (\texttt{time(NULL)}), assurant que les tests couvrent un espace de scalaires varié et réaliste.
\end{itemize}
...
Les mesures ont été réalisées via le fichier \texttt{main.c} sur une machine standard. Le protocole est le suivant :
\begin{itemize}
...
\begin{tabular}{@{}cccc@{}}
\toprule
\textbf{Essai} & \textbf{Classique (s)} & \textbf{GLV (s)} & \textbf{Gain (Speedup)} \\ \midrule
Test 1 & 0.016444 & 0.009070 & \textbf{1.81 x} \\
Test 2 & 0.016345 & 0.009838 & \textbf{1.66 x} \\
Test 3 & 0.016428 & 0.009646 & \textbf{1.70 x} \\
Test 4 & 0.017796 & 0.010455 & \textbf{1.70 x} \\ \midrule
\textbf{Moyenne} & \textbf{0.016753} & \textbf{0.009752} & \textbf{1.72 x} \\ \bottomrule
\end{tabular}
\end{table}

Nous observons une accélération moyenne stable de \textbf{1.72x}, ce qui signifie que la méthode GLV est 72\% plus rapide que la méthode standard.

\subsection{Interprétation de la Performance}
Le gain mesuré (1.72x) est excellent, bien que légèrement inférieur au facteur théorique (2.0x). Cette différence s'explique par les coûts fixes inhérents à notre architecture logicielle, visibles dans \texttt{EC\_GLV.c} :
...
Les résultats expérimentaux sont particulièrement satisfaisants :
\begin{itemize}
    \item Nous avons mesuré une accélération moyenne de \textbf{1.72x} (gain de 72\%) par rapport à la méthode classique.
