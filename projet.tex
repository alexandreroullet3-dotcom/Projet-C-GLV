...
Une clé ECC de 256 bits offre une résistance comparable à une clé RSA de 3072 bits, rendant l'ECC idéale pour les environnements contraints en termes de mémoire (cartes à puces) et les protocoles haute performance (Blockchain).
...
La méthode GLV s'applique aux courbes définies sur un corps fini $\mathbb{F}_p$ de cardinal $\#E(\mathbb{F}_p)=h\cdot n$, avec $n$ premier et $h$ un cofacteur petit ($h\leq 4$), où l'anneau des endomorphismes est isomorphe à un ordre dans un corps quadratique imaginaire $\mathbb{Q}(\sqrt{-D})$.
Concrètement, cela signifie qu'il existe une application rationnelle non-triviale $\phi : E \to E$ qui est "rapide" à calculer et qui vérifie une équation caractéristique :
\[ \phi^2 - T\phi + N = 0 \]
...
\begin{remarque}
    La condition $\#E(\mathbb{F}_p)=h\cdot n$ avec $n$ premier et $h$ un cofacteur petit, est nécéssaire pour des raisons de sécurité. On a besoin du $n$ le plus grand possible pour maximiser le cardinal du sous-groupe sur lequel on utilise GLV.
\end{remarque}
...
\begin{itemize}
    ...
    \item $\lambda$ est l'entier vérifiant la même équation caractéristique dans le groupe $\mathbb{Z}/n\mathbb{Z}$.
\end{itemize}

En résumé, les points vivent dans un groupe cyclique d'ordre $n$, alors que leurs coordonées sont dans $\mathbb{F}_p$.
...
\end{equation}
Avec les constantes $\xi = \frac{-1 + \sqrt{-7}}{2}$ et $a = \frac{\xi-3}{4}$.
\begin{itemize}
...
\end{equation}
où le terme $\sqrt{-2}$ désigne une racine carrée de $-2$ fixée dans $\mathbb{F}_p$.
...
Si nous trouvons un tel $v$, alors le vecteur différence $u = (k, 0) - v $ sera court (sa norme euclidienne est petite), et par construction, si on note $u=(k_1, k_2),\, v= (x, y)$, on a :
\[ k_1 + k_2\lambda \equiv (k - x) + (-y)\lambda \equiv k - (x + y\lambda) \equiv k \pmod n \]
...
L'algorithme d'Euclide Étendu (EEA) appliqué à $n$ et $\lambda$ génère des suites telles que $$s_in+t_i\lambda=r_i,$$ avec la suite des restes $r_i$ strictement décroissants. Nous arrêtons l'algorithme lorsque le reste $r_i$ devient inférieur à $\sqrt{n}$.
...
\textbf{Observation importante :} si $\gcd(n, \lambda) = d$, alors le dernier reste non nul de l'EEA vaut $d$. Dans la pratique GLV, on a toujours $n$ premier, et la suite peut donc toujours descendre jusqu'à un reste $r_m < \sqrt{n}$.  
...
- Pour le vecteur $v_2 = (r_{m-1}, -t_{m-1})$, on a $r_{m-1} > r_m$ mais $r_{m-1} < n$. Dans la pratique la norme est toujours de l’ordre $\sqrt{n}$, sauf sur certains exemples pathologiques.
...
On cherche $v = c_1 v_1 + c_2 v_2 \approx (k, 0)$, le vecteur du réseau le plus proche.
...
\subsection{Algorithme}
Maintenant le $k$ décomposé, la méthose naïve serait de calculer $k_1P$ et $k_2\phi(P)$ par exponentiation rapide puis d'additionner. Cette méthode ne donnerait aucun gain car nous ferions successivement 2 calculs prenant chacun 2 fois moins de temps. Plutôt, nous appliquons l'algorithme suivant dit \textit{Shamir's trick}, qui nous permet de paralléliser les calculs.
\newline

\begin{algorithm}[H]
\SetAlgoLined
\KwData{$w$ paramètre de fenêtrage, $ k, \, l, \,P,\,Q,$ avec $k,l$ des entiers de $t$ bits}
\KwResult{$kP+lQ$}
Précalculer $iP+jQ$, pour $0\le i,j<2^w$\;
Ecrire $k=(k_{d-1},\cdots,k_0), \,l=(l_{d-1},\cdots,l_0)$, où $k_i$ et $l_i$ sont des chaînes de $w$ bits et $d=\lceil t/w\rceil.$\;
$R\leftarrow\mathcal{O}$\;
\For{$i=d-1$ \textbf{downto} $0$}{
    $R\leftarrow2^wR$\;
    $R\leftarrow R+k^iP+l^jQ$\;
}
\caption{Calcul parallélisé de $kP+lQ$}
\end{algorithm}
...
Pour calculer la combinaison linéaire $k_1P + k_2Q$, nous n'effectuons pas deux multiplications séparées. Nous utilisons une méthode d'entrelacement (Interleaving) combinée à du \textbf{fenêtrage (Windowing)} de taille $w$, présenté dans l'algorithme 2. Le paramètre $w$ doit être choisi judicieusement, plus il est grand, moins la boucle principale aura d'itérations, mais plus le temps de précalcul sera long (dans notre projet, $w=2$ donne la meilleur accélération).



\textbf{Note: } La meilleure amélioration de notre programme se trouve ici. Nous précalculons $iP+jQ$, pour $0\leq i,j<2^w$ soit une table de $2^{2w}$ éléments, mais il aurait été possible de seulement précalculer $iP$, pour $0\leq i<2^w$ puis d'utiliser que $jQ=j\phi(P)=\phi(jP)$. La table de precalcul aurait alors seulement $2^w$ éléments. Le choix optimale de $w$ serait alors $w=4$ ou 5. Nous avons remarqué cette optimisation seulement à la fin et n'avons pas eu le temps de l'implémenter.
...
\begin{itemize}
    ...
    \item Ils exportent ces paramètres sous forme de chaînes hexadécimales, qui sont ensuite "codées en dur" dans le fichier \texttt{glv\_curves.c}.
\end{itemize}
...
Le gain mesuré (1.76x) est excellent, bien qu'inférieur au facteur théorique (2.0x). Cette différence s'explique par les coûts fixes inhérents à notre architecture logicielle, visibles dans \texttt{EC\_GLV.c} :

\begin{enumerate}
    \item \textbf{Gain théorique :} Le facteur 2.0x est théorique et asymptotique. Il ne tient pas compte de la réalité physiques des processeurs.
...
Nous avons mesuré une accélération moyenne de \textbf{1.76x} (gain de 76\%) par rapport à la méthode classique.
